...
# GLOBAL MASK CACHE - Stores boolean masks to save 75% VRAM vs float32
_ATTN_MASK_CACHE = {}
...
class WindowAttention4D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask is not None:
            nw = mask.shape[0]
            # Reshape for broadcast addition
            attn = attn.view(b_ // nw, nw, self.num_heads, n, n)
            # Use masked_fill_ with boolean mask: extremely fast and memory efficient
            attn.masked_fill_(mask.unsqueeze(1).unsqueeze(0), float("-100.0"))
...
class SwinTransformerBlock4D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        x = F.pad(x, (0, 0, pad_t0, pad_t1, pad_w0, pad_w1, pad_h0, pad_h1, pad_d0, pad_d1))
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Get cached boolean mask
        attn_mask = get_attn_mask([dp, hp, wp, tp], window_size, shift_size, x.device)
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers - 1):
            layer = BasicLayer(
                ...
                window_size=self.first_window_size if i_layer == 0 else self.window_size,
